...
`ClubsResponseDto`에는 `recruitingStatus` 필드가 있다.
상태가 `OPEN`일 때만 `apply` 링크를 포함시키면, **클라이언트가 별도 분기 없이 링크 존재 여부로 버튼 렌더링을 결정**할 수 있다.
...
> `linkTo(methodOn(...))` 은 Spring HATEOAS가 제공하는 정적 메서드.
> `apply` 링크는 `MemberClubController` 경로이므로 하드코딩하거나, `MemberClubController`의 메서드를 참조해도 된다.

### `ClubDetailRepresentationAssembler`
...
public class GuestClubController {
    ...
    public ResponseEntity<ApiResponseDto<ClubResponseDto>> findClubById(@PathVariable Long clubId) {
        ClubResponseDto club = guestClubService.findClubById(clubId);
        clubDetailAssembler.toModel(club); // 링크 추가 (반환값이 같은 객체)
        return ResponseEntity.ok(ApiResponseDto.success(club, "클럽 상세 정보 조회 성공"));
    }
}
```
...
클라이언트는 `response.data._links.apply` 식으로 접근하면 된다.
...
| `ClubsRepresentationAssembler.java` | 신규 생성 — 목록 DTO에 링크 추가 |
| `ClubDetailRepresentationAssembler.java` | 신규 생성 — 상세 DTO에 링크 추가 |
| `GuestClubController.java` | Assembler 주입, 링크 적용 로직 추가 |

---

## 검증 체크리스트

- [ ] `GET /api/v1/public/clubs` 응답의 각 item에 `_links.self`, `_links.list` 포함 여부
- [ ] `recruitingStatus: OPEN`인 item에만 `_links.apply` 포함 여부
- [ ] `recruitingStatus: CLOSED`인 item에 `_links.apply` 미포함 여부
- [ ] `GET /api/v1/public/clubs/{id}` 상세 응답에 동일하게 적용 여부
